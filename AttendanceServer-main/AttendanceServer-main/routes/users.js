...
router.post("/login", async (req, res) => {
  const datauser = await user.find({}).select("status name face_data user_id company_id root department_id phone day_of_birth password avatar money" );
...
router.post("/getUserList", async (req, res) => {
  try {
    ...
    const data = await user.find({company_id:company_id}).select(" name face_data user_id company_id root department_id phone day_of_birth password avatar" );
    res.send({
      code: 200,
      data: data,
      message: "success",
...
router.post("/findUserById", async (req, res) => {
  try {
    let user_id = req.body.user_id
    const data = await user.find({user_id:user_id}).select(" name face_data user_id company_id root department_id phone day_of_birth password avatar" );
    res.send({
      code: 200,
      data: data,
      message: "success",
...
router.post("/getUserByDepartment", async (req, res) => {
  try {
    ...
    let company_id = req.body.company_id
    const data = await userController.getUserByDetpartment(department_id, company_id);
    res.send({
      code: 200,
      data: data,
      message: "success",
